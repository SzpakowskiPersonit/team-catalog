...
3. Run the golden cases: `claude plugin eval` over `evals/`. Record which cases fail and
   on which model. A case that fails is evidence; a procedure nobody ran is not.
...
6. Write the report in the Output format. Open one pull request per retire, and one for all
   the re-verifies together — a date bump does not need its own review.
